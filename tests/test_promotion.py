...
release_builder = _load(
    "claude_promotion_release_builder",
    ROOT / "tools" / "release_builder.py",
)
finalizer = _load(
    "claude_promotion_finalizer",
...
def test_promotion_preserves_exact_candidate_zip(tmp_path: Path):
    ...
    source_zip = candidate / "claude-base-0.1.0.zip"
...
def test_promotion_fails_closed_on_canary_failure(tmp_path: Path):
    candidate, binding = _candidate(tmp_path)

    with pytest.raises(ValueError, match="CLAUDE_CANARY"):
