...
def _load_json_bytes(payload: bytes) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError("session tools manifest is not valid UTF-8 JSON") from error
...
def validate_session_tools_archive(
    ...
    *,
    ...
    try:
        ...
        raise ValueError("session tools ZIP is unreadable") from error
    return manifest
...
def build_session_tools_bundle(
    ...
    *,
        ...
        _json_bytes(
            {
                "schema_version": 1,
                "target": "claude",
                "release_tag": f"claude-v{base_version}",
    ...
    zip_path = dist_root / f"session-tools-claude-{base_version}.zip"
...
def validate_session_tools_asset_record(value: object) -> dict[str, object]:
    ...
    name = value["name"]
    if not isinstance(name, str) or not re.fullmatch(r"session-tools-claude-[^/\\]+\.zip", name):
