...
def _validate_provider_marker(marker: dict[str, Any]) -> None:
    ...
    eligibility = marker.get("eligibility")
    valid = (
        marker.get("schema_version") == 1
        and marker.get("target") == TARGET
        and marker.get("client") == VERSIONED_CLIENT
        and marker.get("provider") == "anthropic"
        and marker.get("model") == "sonnet"
        and marker.get("effort") == "low"
        and marker.get("tools") == "disabled"
        and marker.get("session_persistence") is False
        and marker.get("calls_authorized") == 1
        and marker.get("calls_completed") == 1
        and marker.get("CLAUDE_PROVIDER_MARKER") == "PASS"
        and _valid_usage(marker.get("usage"))
        and _valid_sha256(marker.get("result_sha256"))
        and isinstance(eligibility, dict)
        and _valid_sha256(eligibility.get("sha256"))
        and eligibility.get("max_age_days") == 7
        and isinstance(eligibility.get("supported_regions_source"), str)
        and isinstance(eligibility.get("consumer_terms_source"), str)
        ...
        and privacy.get("account_identifier_included") is False
        and _valid_body(marker)
    )
    if not valid:
        raise ValueError("Claude provider marker evidence is invalid")
...
def _validate_canary(
    ...
    if not valid:
        raise ValueError("Claude canary evidence is invalid or unbound")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Compose pre-publication Claude FULL evidence from an accepted "
            "candidate, the one authorized marker, and a live canary."
        )
    )
