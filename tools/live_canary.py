...
release_builder = _load_tool(
    "claude_canary_release_builder",
    ROOT / "tools" / "release_builder.py",
)
offline_runner = _load_tool(
    "claude_canary_offline_runner",
...
def _load_candidate(candidate_dir: Path):
    ...
    if (
        ...
        or manifest.get("tag") != f"claude-v{manifest.get('version')}"
        ...
        raise ValueError("Claude candidate evidence is invalid")
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("dist/claude-live-canary.json"),
    )
