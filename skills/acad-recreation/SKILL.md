...
```
Скрипт (idempotent): копирует toolkit в `C:\ProgramData\ORG-acad` (ASCII-путь, т.к. профиль
...
| `tools/autocad_config_org.py` | Эталон `config.py`: backend-priority (живой AutoCAD + autostart, ezdxf только явный). install.ps1 копирует поверх upstream | применяется install.ps1 |
...
- **Безопасный блок:** `(ORG:safe-run '(...))` — оборачивает геометрию в `UNDO _GROUP … _END` +
  финальный `ZOOM _E`, чтобы один откат снимал весь шаг (канонический скелет из ClaudeCAD-методологии).
- **Слои:** `(ORG:ensure-layers '(("!Приток" 1)("!Вытяжка" 5)("!Рециркуляция" 6)))` — создать/настроить.
...
- **Ориентация динблоков** (точка вставки = угол, не ось): круглый `Воздуховод`
  ...
  Helpers `ORG:duct-cl`/`ORG:rect-cl` определять каждую сессию (геометрия-специфика, не в toolkit).
