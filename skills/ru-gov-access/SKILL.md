---
name: ru-gov-access
description: Use when нужен российский госреестр с гео-ограничениями.
---
...
- **с иностранного egress** (VPN/Дубай) — playwright через RU-прокси (живой RU-SOCKS5 из вывода `ru_fetch.py`/`$RU_PROXY`):
  ...
  ⚠ Проверено вживую 2026-06-23 (машина с ИНОСТРАННЫМ egress — ноутбук, egress=Дубай AS13335): локальный playwright БЕЗ RU-прокси на
  ...
  ⚠ ВАЖНО — НЕ путать «playwright не открыл» с «сайт недоступен»: СТРАНИЦУ pub.fsa с ТОГО ЖЕ Дубай-egress
  `ru_fetch.py` берёт за `HTTP 200` (через RU-SOCKS5, авто-нашёл живой `147.45.146.97:1080`) — проверено в
  этой же сессии 2026-06-23. Голый playwright не годится НЕ потому что сайт недоступен, а потому что идёт
  БЕЗ RU-proxy. Для СТРАНИЦ/PDF канал — `ru_fetch.py` (работает); playwright `--proxy-server=socks5://<RU>`
  нужен только для DATA-слоя (SPA-поиск), который `ru_fetch` curl-ом не возьмёт.
